...
describe("Telnyx campaign surface consistency", () => {
  it("keeps the public CTA and SMS program document aligned with the shared program contract", async () => {
    const [smsTerms, program] = await Promise.all([
      readFile("public/sms.html", "utf8"),
      readFile("../../docs/SMS_PROGRAM.md", "utf8"),
    ]);

    expect(smsTerms).toContain(`<strong>${TELNYX_SMS_PROGRAM_NAME}</strong>`);
    expect(program).toContain(`Product and SMS program: \`${TELNYX_SMS_PROGRAM_NAME}\``);
    expect(program).toContain(TELNYX_SMS_CONSENT_DISCLOSURE);
    for (const category of TELNYX_SMS_CATEGORIES) {
      expect(TELNYX_SMS_CATEGORY_LABELS[category]).toBeTruthy();
    }
    const samples = program.match(/^> Film by Dust Wave:.*$/gm) ?? [];
