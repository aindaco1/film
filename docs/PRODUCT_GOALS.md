...
| Static-first, framework-light UI | The browser app remains TypeScript and DOM rendering without a UI framework. Shared renderers and client modules own repeated behavior; local-handoff, call-sheet/sides, production-resource, and screenplay-review serializers load behind four tested domain boundaries. | On track; extraction passes removed 930 lines from `main.ts` and 25.7 kB from the entry chunk. The remaining 18,733-line module and 608,168-byte entry warning remain maintainability risks |
...
| Common-flow UX quality | The canonical inventory covers 51 user flows across all application workspaces, with 595 automated tests plus browser, accessibility, mobile-boundary, and real-local-Worker gates. Visible project/task/person/equipment/expense editors now queue and replay the same operations they display. | On track and locally verified |
