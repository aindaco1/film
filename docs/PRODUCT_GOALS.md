...
| Static-first, framework-light UI | The browser app remains TypeScript and DOM rendering without a UI framework. Shared renderers and client modules own repeated behavior; call-sheet/sides, production-resource, and screenplay-review serializers now load behind three tested domain boundaries. | On track; two extraction passes removed 606 lines from `main.ts` and about 18.5 kB from the entry chunk, but the remaining 19k-line module and over-500 kB warning remain maintainability risks |
