...
| Static-first, framework-light UI | The browser app remains TypeScript and DOM rendering without a UI framework. Shared renderers and client modules own repeated behavior; call-sheet and sides serializers now load behind one tested production-document boundary. | On track; the first lazy boundary reduced the entry chunk, but large `main.ts` and the over-500 kB bundle warning remain maintainability risks |
